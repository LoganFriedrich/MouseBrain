...
class CurationWidget(QWidget):
    ...
    def start_review_mode(self):
        ...
        if self.sample_check.isChecked():
            ...
            print(f"[SCI-Connectome] Sampling {n_sample} candidates ({self.sample_spin.value()}%) for QC review")
        else:
    ...
    def _save_points_to_xml(self, points, cell_type: str) -> Path:
        ...
        try:
            ...
            print(f"[SCI-Connectome] Saved {len(points)} {cell_type} to {save_path}")
            return save_path

        except Exception as e:
            print(f"[SCI-Connectome] Error saving to XML: {e}")
            import traceback
            traceback.print_exc()
            return None
            ...
            for widget in self.viewer.window.dock_widgets.values():
                if hasattr(widget, 'widget'):
                    ...
                    if hasattr(w, 'session_doc') and w.session_doc is not None:
                        ...
                        print(f"[SCI-Connectome] Logged curation to session documenter")
                        return
        except Exception as e:
            print(f"[SCI-Connectome] Could not log to session: {e}")
