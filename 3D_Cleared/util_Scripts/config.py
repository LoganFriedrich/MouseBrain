#!/usr/bin/env python3
"""
config.py - Central configuration for SCI-Connectome pipeline paths.
...
3. Fallback to Y:\\LAB_ROOT (for backwards compatibility with lab setup)

Usage in other scripts:
    from config import BRAINS_ROOT, SCRIPTS_DIR, MODELS_DIR, DATA_SUMMARY_DIR
    ...
    for _ in range(10):  # Max 10 levels up to prevent infinite loop
        ...
        if current.name == "util_Scripts":
            ...
            if parent.name == "3D_Cleared":
                # New structure: Tissue/3D_Cleared -> return Tissue's parent (LAB_ROOT)
