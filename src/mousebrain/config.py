#!/usr/bin/env python3
"""
config.py - Central configuration for Connectome pipeline paths (mousebrain package).
...
4. Fallback to Y:\\2_Connectome (for backwards compatibility with lab setup)
...
Environment Variable Override:
    Set CONNECTOME_ROOT to the root of your installation:

    Windows (cmd):
        set CONNECTOME_ROOT=C:\\Projects\\Connectome

    Windows (PowerShell):
        $env:CONNECTOME_ROOT = "C:\\Projects\\Connectome"

    Linux/Mac:
        export CONNECTOME_ROOT=/home/user/Connectome

    Legacy (still supported):
        set SCI_CONNECTOME_ROOT=C:\\Projects\\Connectome
"""
...
def _get_root_path() -> Path:
    ...
    if env_root:
        ...
        if env_path.exists():
            ...
            warnings.warn(
                f"CONNECTOME_ROOT is set to '{env_root}' but path doesn't exist. "
...
def print_config():
    ...
    print("=" * 60)
    print("Connectome Configuration")
    print("=" * 60)

    env_var = os.environ.get("CONNECTOME_ROOT") or os.environ.get("SCI_CONNECTOME_ROOT") or "(not set)"
    print(f"\nEnvironment variable CONNECTOME_ROOT: {os.environ.get('CONNECTOME_ROOT', '(not set)')}")
    print(f"Environment variable SCI_CONNECTOME_ROOT (legacy): {os.environ.get('SCI_CONNECTOME_ROOT', '(not set)')}")
