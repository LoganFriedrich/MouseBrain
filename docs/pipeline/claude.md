...
> **Navigation**: [← Back to Project Root](../../../../claude.md)

---

## STOP: This is a DATA directory, not the code repository

**DO NOT create new `.py` files here. DO NOT add features to existing scripts here.**

The canonical code lives in `Tissue/MouseBrain/src/mousebrain/` — that's the git repo and pip-installable package. This directory contains only:
- **Legacy pipeline scripts** (1-6_*.py) that import from `mousebrain`
- **Backward-compat shims** (`config.py`, `experiment_tracker.py`) that contain ONLY `from mousebrain.xxx import *`

If you need to fix a bug or add a feature, edit the source in `MouseBrain/src/mousebrain/`, not here.

---

## Overview

This folder contains legacy Python scripts for the Connectome cell detection pipeline. They run ON the data and import from the `mousebrain` package.

**Key principle**: The tracker (`mousebrain.tracker`) is essential - it records ALL calibration runs.
...
This workstation has **104 logical cores**. Any script that accepts `--n-free-cpus` (e.g., `util_train_model.py`, cellfinder detection/classification) should use:

```
--n-free-cpus 74
```

The default of 2 would spawn 102 workers, which is wasteful since GPU is the bottleneck. 74 free leaves 30 worker cores — more than enough for data loading.
...
- Canonical napari plugin code: `Tissue/MouseBrain/src/mousebrain/plugin_3d/`
